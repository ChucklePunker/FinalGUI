...
public class TextMenu extends JMenu {
	...
	private final JCheckBoxMenuItem bold;   // controls whether the text is bold or not.
	private final JCheckBoxMenuItem italic; // controls whether the text is italic or not.
	...
	public TextMenu(DrawPanel owner) {
		...
		final JMenuItem lineHeight = new JMenuItem("Line Height");
		lineHeight.addActionListener(e -> {
			double currentLineHeight = (panel.getTextItem().getLineHeightMultiplier());
			String l = JOptionPane.showInputDialog(panel, "What font size do you want to use?",currentLineHeight);
			try{
				double newLineHeight = Double.parseDouble(l);
				panel.getTextItem().setLineHeightMultiplier(newLineHeight);
				panel.repaint();
			} catch (Exception ex) {
				JOptionPane.showMessageDialog(panel, "Please enter a legal line height\nUse a decimal number");
			}
		});
